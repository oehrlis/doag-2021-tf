...
resource "oci_core_nat_gateway" "natgw" {
  ...
  display_name   = var.label_prefix == "none" ? "${local.resource_shortname}_natgw" : "${var.label_prefix} ${local.resource_shortname}_natgw"
...
resource "oci_core_route_table" "private_route_table" {
  compartment_id = oci_identity_compartment.demo03.id
  display_name   = var.label_prefix == "none" ? "${local.resource_shortname} private route" : "${var.label_prefix} ${local.resource_shortname} private route"
  vcn_id         = oci_core_vcn.vcn.id
  freeform_tags  = var.tags
